...
PID_ATune::PID_ATune(double* Input, double* Output)
{
	...
	oStep = 30;
	SetLookbackSec(10);
	lastTime = Brain.Timer.time(timeUnits::msec);
}
...
int PID_ATune::Runtime()
{
	...
	unsigned long now = Brain.Timer.time(timeUnits::msec);
  ...
  { //we've transitioned.  check if we can autotune based on the last peaks
    double avgSeparation = (std::abs(peaks[peakCount-1]-peaks[peakCount-2])+std::abs(peaks[peakCount-2]-peaks[peakCount-3]))/2;
