...
void ACCESS_OS::clearLine(int l_row) //select what row to clear but not selector if isCursorOn = true
{
	if (isCursorOn == true)
	{
		Controller.Screen.setCursor(l_row + 1, 2);	//Prevent overwrite of selector GUI
		Controller.Screen.print("               "); //15 spaces (1 less than screenTextWidth)
	}
	else
	{
		Controller.Screen.clearLine();	//If isCursorOn = false, wipe line with system clearLine
	}
}
...
void ACCESS_OS::clearScreen() //clears everything but not selector if isCursorOn = true
{
	...
	{
		Controller.Screen.clearScreen();	//If isCursorOn = false, wipe line with system clearScreen
	}
...
void ACCESS_OS::print(std::string text, int row, int col) //pass text, row, and col
{
	...
	if (isCursorOn == true)
	{
		Controller.Screen.setCursor(row + 1, col + 2); //Col + 2: Prevent overwrite of selector GUI
	}
	else
	{
		Controller.Screen.setCursor(row + 1, col + 1); //Col + 1: Start from beginning
	}
	Controller.Screen.print(text.c_str()); //Future GUI will scroll text
}
